...
router.post("/", function(request, response) {
  ...
  let completeJsonResponse = {
    ...
    task: {
      ...
      reference_email: {
        // a mutable version of the original email. The onCommand hook pre-populates these. Your webhook can override them depending on the case (ex: this could represent a future email, not the original email)
        to: [],
        cc: [],
        bcc: [],
        from: "",
    ...
    response: [
      {
        ...
        body: [
          ...
          },
          {
            ...
            body:
              "name: %0A amount: %0A tag [network, colo, hosting]: %0A probability[1-10]: %0A%0A%0A--%0AThis is a Gopher email-action, a handy way of getting stuff done without ever leaving your inbox.",
            style: "block primary"
          },
          {
